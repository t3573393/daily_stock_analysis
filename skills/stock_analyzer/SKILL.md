...
Skill 支持**自动检测**智能体的模型配置，无需重复配置 LLM。

### 自动配置（推荐）

Skill 会自动检测并使用智能体的 LLM 配置，**无需重复配置**！

配置优先级（从高到低）：

1. **运行时参数**（代码中传入，最高优先级）
2. **环境变量**（`LLM_API_KEY`, `LLM_MODEL` 等）
3. **Skill 配置文件**（`~/.config/claude/stock_analyzer.yaml`）
4. **智能体默认配置**（自动检测 `~/.config/claude/config.yaml` 等）
5. **默认值**（`openai/gpt-4o`，最低优先级）

**这意味着**：
- ✅ 如果你已经在智能体中配置好了模型（如 Claude Code、Cursor 等），Skill 会自动使用这些配置
- ✅ 如果你需要覆盖某些设置，只需设置环境变量即可
- ✅ 如果你需要完全自定义，可以创建 Skill 专属配置文件
- ✅ 所有配置方式可以共存，高优先级配置会覆盖低优先级配置

### 手动配置（可选）

如果需要自定义配置，可以创建 Skill 专属配置文件：
